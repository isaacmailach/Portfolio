$(document).ready(function () {
	...
	function UpdateModal() {
		...
		modal_header_image.previousElementSibling.setAttribute(
			"srcset",
		...
		$(".page-content-grid-item[data-num=" + current_num + "]").addClass(
			"page-content-grid-item_focus",
		);
		audio.load();
	...
	function UpdateSocialLinks() {
		...
		$(".modal-header-toolbar-share-icon_email").attr(
			"href",
			"mailto:?body=" +
				current_url +
				"%3Fid%3D" +
		...
		$(".modal-header-toolbar-share-icon_facebook").attr(
			"href",
	...
	function SearchItems(phrase) {
		...
		for (var i = 0; i < visible_item_data.length; i++) {
			...
			if (search_item.text().search(new RegExp(phrase, "i")) < 0) {
				...
				search_item.css({
					position: "absolute",
					top: search_item_data.old_pos.top + "px",
					left: search_item_data.old_pos.left + "px",
					width: search_item_data.old_width + "px",
				});
		...
		setTimeout(function () {
			for (var i = 0; i < visible_item_data.length; i++) {
				...
				if (!search_item_data.hidden) {
					search_item.css({
						animation: "search_animate 0.5s cubic-bezier(0.4, 0, 0.2, 1)",
					});
				}
			}
		}, 1);
