$(document).ready(function () {
	...
	const play_button = modal_header_toolbar.querySelector(
		".modal-header-toolbar-player-play",
	);
	const modal_player_time = modal_header_toolbar.querySelector(
	...
	fetch("data/musical-compositions.json")
		...
		.then(function (database) {
			...
			var item_template = document.querySelector(
				".page-content-grid-item_template",
			);
			for (var i = 0; i < database.item.length; i++) {
				...
				if (!temp_item_data.hidden) {
					...
					if (temp_item_data.align_top) {
						item.className += " align-top";
					}
					item.setAttribute("tabindex", 0);
					item.dataset.num = visible_item_data.length - 1;
					item.setAttribute(
						"style",
						"flex-grow: " + parseInt(100 * Math.random()),
					);
					var template = document.importNode(item_template.content, true);
					template.querySelector(".page-content-grid-item-image").src =
						"img/musical-compositions/" + temp_item_data.id + "/image.jpg";
					template
						.querySelector(".page-content-grid-item-image")
						.setAttribute(
							"srcset",
							"img/musical-compositions/" +
								temp_item_data.id +
								"/image.webp, img/musical-compositions/" +
								temp_item_data.id +
								"/image-3x.webp 2x",
						);
					template
						.querySelector(".page-content-grid-item-image")
						.setAttribute("alt", temp_item_data.name);
					template.querySelector(
						".page-content-grid-item-overlay-name",
					).innerHTML = temp_item_data.name_shy;
					template.querySelector(
						".page-content-grid-item-overlay-date",
					).innerText = temp_item_data.date;
					template.querySelector(
						".page-content-grid-item-overlay-instrumentation",
					).innerText = "For " + temp_item_data.instrumentation;
	...
	document
		.querySelector(".alert_no-id .alert-content-close")
		.addEventListener("click", function () {
			CloseAlert("no-id");
		});
	document
		.querySelector(".alert_no-load .alert-content-close")
		.addEventListener("click", function () {
			CloseAlert("no-load");
		});

	document
		.querySelector(".page-content-search-icon")
		.addEventListener("click", function () {
			search_input.focus();
		});
	...
	document
		.querySelector(".modal-header-arrow_left")
		.addEventListener("click", function () {
	...
	document
		.querySelector(".modal-header-arrow_right")
		.addEventListener("click", function () {
	...
	audio.onprogress = function () {
		let audio_buffer_end = 0;
		if (audio.buffered.length > 0) {
			audio_buffer_end = audio.buffered.end(audio.buffered.length - 1);
		} else {
			audio_buffer_end = 0;
		}
		modal_header_toolbar.querySelector(
			".modal-header-toolbar-player-progress-loaded",
		).style.width = (audio_buffer_end / audio.duration) * 100 + "%";
	};
	...
	modal_header_toolbar
		.querySelector(".modal-header-toolbar-player-progress")
		.addEventListener("click", function (e) {
			audio.currentTime =
				((e.pageX - this.getBoundingClientRect().left) / this.clientWidth) *
				audio.duration;
		});

	function UpdateAudioTime() {
		modal_player_time.innerText =
	...
	function UpdateModal() {
		...
		const modal_header_image = modal.querySelector(".modal-header-image");
		var modal_body = modal.querySelector(".modal-body");
		modal_header_image.src =
			"img/musical-compositions/" + current_id + "/cover.webp";
		modal_header_image.previousSibling.setAttribute(
			"srcset",
			"img/musical-compositions/" + current_id + "/image-3x.webp",
		);
		...
		modal_body.innerHTML =
			"<h3>" +
			visible_item_data[current_num].name_shy +
			"</h3><small>" +
			visible_item_data[current_num].date +
			"</small>" +
			"<h4>For " +
			visible_item_data[current_num].instrumentation +
			"</h4>";
		if (current_data.credit) {
			modal.querySelector(".modal-credit").textContent =
				"Image credit: " + current_data.credit + ".";
		}
		document
			.querySelector('.page-content-grid-item[data-num="' + current_num + '"]')
			.classList.add("page-content-grid-item_focus");
		audio.load();
		if (current_data.light) {
			modal.querySelector(".modal-header").classList.add("light");
		} else {
			modal.querySelector(".modal-header").classList.remove("light");
		}
		UpdateSocialLinks();
		fetch("text/musical-compositions/" + current_id + ".html")
			...
			.then(function (text) {
				...
				if (visible_item_data[current_num].video) {
					modal_body.innerHTML +=
						'<div class="modal-body-video"><iframe src="https://www.youtube.com/embed/' +
						visible_item_data[current_num].video +
	...
	function ClearModal() {
		...
		modal.querySelector(".modal-credit").textContent = "";
		document
			.querySelector(".page-content-grid-item_focus")
			.classList.remove("page-content-grid-item_focus");
	}
	function ResetModal() {
	...
	function CheckFirstLast() {
		let arrow_left = document.querySelector(".modal-header-arrow_left");
		let arrow_right = document.querySelector(".modal-header-arrow_right");
		if (current_num == 0) {
			next_item = false;
			arrow_left.classList.add("modal-header-arrow_disabled");
			arrow_left.tabIndex = -1;
		} else {
			next_item = true;
			arrow_left.classList.remove("modal-header-arrow_disabled");
			arrow_left.tabIndex = 0;
		}
		if (current_num == visible_item_data.length - 1) {
			previous_item = false;
			arrow_right.classList.add("modal-header-arrow_disabled");
			arrow_right.tabIndex = -1;
		} else {
			previous_item = true;
			arrow_right.classList.remove("modal-header-arrow_disabled");
			arrow_right.tabIndex = 0;
		}
	}
	function OpenModal() {
		CheckFirstLast();
		...
		if (next_item) {
			...
			previous_img.src =
				"img/musical-compositions/" +
				visible_item_data[parseInt(current_num) - 1].id +
				"/cover.webp";
		}
		if (previous_item) {
	...
	function CloseModal() {
		let focus_item = document.querySelector(
			'.page-content-grid-item[data-num="' + current_num + '"]',
		);
	...
	function UpdateSocialLinks() {
		...
		document
			.querySelector(".modal-header-toolbar-share-icon_email")
			.setAttribute(
				"href",
				"mailto:?body=" +
					current_url +
		...
		document
			.querySelector(".modal-header-toolbar-share-icon_facebook")
			.setAttribute(
				"href",
	...
	if (!navigator.share) {
		document.querySelector(
			".modal-header-toolbar-share-icon_share",
		).style.display = "none";
	} else {
		document
			.querySelector(".modal-header-toolbar-share-icon_share")
	...
	function OpenAlert(type) {
		let alert = document.querySelector(".alert_" + type);
	...
	function UpdateQueries() {
		...
		window.history.replaceState(
			"",
			"Isaac Mailach - Music" + (query.id ? " - " + current_data.name : ""),
	...
	function SearchItems(phrase) {
		...
		for (var i = 0; i < visible_item_data.length; i++) {
			var search_item = document.querySelector('[data-num="' + i + '"]');
			var search_item_data = visible_item_data[i];
			search_item_data.old_pos = {
				top: search_item.offsetTop,
				left: search_item.offsetLeft,
			};
			search_item_data.old_width = search_item.offsetWidth;
			if (!search_item_data.hidden) {
				search_item_data.animate = true;
				search_item.style.animation = "";
				search_item.style.transform = "";
				search_item.style.transition = "";
			} else {
		...
		for (var i = 0; i < visible_item_data.length; i++) {
			var search_item = document.querySelector('[data-num="' + i + '"]');
			var search_item_data = visible_item_data[i];
			if (search_item.innerText.search(new RegExp(phrase, "i")) < 0) {
				search_item_data.hidden = true;
				search_item.style.position = "absolute";
				search_item.style.top = search_item_data.old_pos.top + "px";
				search_item.style.left = search_item_data.old_pos.left + "px";
				search_item.style.width = search_item_data.old_width + "px";
				search_item.classList.add("hide_search");
			} else {
				search_item_data.hidden = false;
				search_item.style.position = "";
				search_item.style.top = "";
				search_item.style.left = "";
				search_item.style.width = "";
			}
		}
		...
		for (var i = 0; i < visible_item_data.length; i++) {
			...
			var search_item_data = visible_item_data[i];
			if (!search_item_data.hidden) {
				search_item_data.new_width = search_item.offsetWidth;
				search_item.classList.remove("hide_search");
				if (search_item_data.animate) {
					search_item_data.new_pos = {
						top: search_item.offsetTop,
						left: search_item.offsetLeft,
					};
					search_item.style.transform =
						"translate(" +
						(search_item_data.old_pos.left - search_item_data.new_pos.left) +
						"px, " +
						(search_item_data.old_pos.top - search_item_data.new_pos.top) +
						"px) scale(" +
						search_item_data.old_width / search_item_data.new_width +
						", 1)";
					search_item.style.transition = "none";
				}
			}
		}
		...
		setTimeout(function () {
			for (var i = 0; i < visible_item_data.length; i++) {
				...
				if (!search_item_data.hidden) {
					search_item.style.animation =
						"search_animate 0.5s cubic-bezier(0.4, 0, 0.2, 1)";
				}
			}
		}, 1);
		// Clear animations
    ...
    function UpdateModal () {
        ...
        audio_sources[0].src = 'audio/musical-compositions/' + current_id + '.mp3';
        audio_sources[1].src = 'audio/musical-compositions/' + current_id + '.ogg';
        modal_body.innerHTML = '<h3>' + visible_item_data[current_num].name_shy + '</h3><small>' + visible_item_data[current_num].date + '</small>' + '<h4>For ' + visible_item_data[current_num].instrumentation + '</h4>';
        if (current_data.credit) {
            modal.querySelector('.modal-credit').textContent = 'Image credit: ' + current_data.credit + '.';
        }
        ...
        fetch('text/musical-compositions/' + current_id + '.html')
            ...
            .then(function (text) {
                modal_body.innerHTML += text;
                if (visible_item_data[current_num].video) {
                    modal_body.innerHTML += '<div class="modal-body-video"><iframe src="https://www.youtube.com/embed/' + visible_item_data[current_num].video + '" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>';
                }
            })
    }
    function ClearModal () {
        ...
        setTimeout(function () {
            document.querySelector('.modal-header-toolbar-player-progress-played').style.width = 0;
            document.querySelector('.modal-header-toolbar-player-progress-loaded').style.width = 0;
        }, 1);
        modal.querySelector('.modal-body').innerHTML = '';
        modal.querySelector('.modal-credit').textContent = '';
        document.querySelector('.page-content-grid-item_focus').classList.remove('page-content-grid-item_focus');
    }
    function ResetModal () {
        modal.classList.add('hide');
        setTimeout(function () {
